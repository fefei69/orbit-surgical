...
# from omni.isaac.sensor import TiledCameraCfg
...
def main():
    ...
    # This state machine is only for the absolute task
    args_cli.task = "Isaac-Lift-Needle-PSM-IK-Abs-v0"
    ...
    with torch.inference_mode(): 
        while not collector_interface.is_stopped():
            ...
            # -- images, should be (1, H, W, C) and np.uint8, multivie: (N, H, W, C) 
            img_front = env.unwrapped.scene["tiled_camera_front"].data.output["rgb"].squeeze().cpu().numpy().astype("uint8")
            img_back = env.unwrapped.scene["tiled_camera_back"].data.output["rgb"].squeeze().cpu().numpy().astype("uint8")
            img_left = env.unwrapped.scene["tiled_camera_left"].data.output["rgb"].squeeze().cpu().numpy().astype("uint8")
            img_right = env.unwrapped.scene["tiled_camera_right"].data.output["rgb"].squeeze().cpu().numpy().astype("uint8")
            ...
            # obs/images/tiled_camera_back is not allowed in Robomimic
            collector_interface.add(f"obs/tiled_camera_front", img_front)
            collector_interface.add(f"obs/tiled_camera_back", img_back)
            collector_interface.add(f"obs/tiled_camera_left", img_left)
            collector_interface.add(f"obs/tiled_camera_right", img_right)
            
            # robomimic only cares about policy observations
            # store signals from the environment
            # -- next_obs no need for bc
            # for key, value in obs_dict["policy"].items():
            ...
            collector_interface.add("success", dones)
